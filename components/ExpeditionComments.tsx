...
export default function ExpeditionComments({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError("");
    const res = await fetch(`/api/expeditions/${expeditionId}/comments`, {
      method: "POST",
  ...
  async function handleDelete(commentId: string) {
    await fetch(`/api/expeditions/${expeditionId}/comments`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    ...
    <section style={{ marginTop: "48px", paddingTop: "40px", borderTop: "1px solid rgba(74,59,42,0.3)" }}>
      ...
      {comments.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "32px" }}>
          {comments.map((c) => {
            ...
            return (
              <div key={c.id} style={{ background: "#1a1a1a", border: "1px solid rgba(74,59,42,0.3)", padding: "14px 18px" }}>
      ...
      {currentUserId ? (
        ...
        <p className="font-body text-muted-ink" style={{ fontSize: "0.82rem" }}>
          <Link href="/login" className="text-neon-green hover:text-chaos-orange transition-colors duration-150">Sign in</Link>{" "}
          to leave a note.
        </p>
      )}
    </section>
