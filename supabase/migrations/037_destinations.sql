...
CREATE INDEX destinations_location_idx ON destinations (location_id);
...
CREATE TRIGGER locations_set_slug BEFORE INSERT ON locations
  FOR EACH ROW EXECUTE FUNCTION public.set_slug('name');
CREATE TRIGGER destinations_set_slug BEFORE INSERT ON destinations
  FOR EACH ROW EXECUTE FUNCTION public.set_slug('name');

CREATE TRIGGER locations_record_slug_change AFTER UPDATE OF slug ON locations
  FOR EACH ROW EXECUTE FUNCTION public.record_slug_change('location');
CREATE TRIGGER destinations_record_slug_change AFTER UPDATE OF slug ON destinations
...
ALTER TABLE expeditions ADD COLUMN destination_id uuid REFERENCES destinations(id) ON DELETE SET NULL;
ALTER TABLE stories ADD COLUMN destination_id uuid REFERENCES destinations(id) ON DELETE SET NULL;
CREATE INDEX expeditions_destination_idx ON expeditions (destination_id);
CREATE INDEX stories_destination_idx ON stories (destination_id);
